...
## v26.1-28.0 (2026-08-25) - LTS 发布

### 新增
- 导航轴球 gizmo (视口右上角): 六个可点击轴球 (±X/±Y/±Z) 带轴线,
  深度排序渲染, 拖拽背景轨道旋转; 点击即切换到对应轴视图
  (Blender/Unity 惯例)
- 拖拽变换实时读数: 状态栏显示 `Dx: 1.20 Dy: 0.00 Dz: -0.50` (移动),
  `Scale: x/y/z` (缩放), `D-angle: 45.0°` (旋转) — Blender 头部行为
- 拖拽中途轴锁定: 拖拽期间按 X/Y/Z 将变换约束到该轴 (无需预选)
- 拖拽中途精调: Shift = 0.1 倍增量率
- 拖拽中途数值输入: 键入数字/./- 累计精确值, 逐键实时应用到约束轴
  (Blender 模态算子行为)
- 层级双击内联重命名 (Outliner 惯例)
- Gizmo 拖拽键盘隔离: 拖拽期间屏蔽全部快捷键 (防止 '1'/'2'/'3' 在
  变换中意外生成图元)

### 测试
- 7 个新 Blender 交互保真 E2E 测试
- 全量套件: 367 E2E 通过 / 0 失败 + 6 单元通过
